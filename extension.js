...
async function markdownPdf(option_type) {
  ...
  try {
    ...
    var filenameWithExt = mdfilename.replace(/^.*[\\\/]/, '');
    ...
    var filenameInput = "";
    while (!filenameInput.trim().length || !isFilenameValid(filenameInput)) {
      filenameInput = await askForInput(filenameWithExt.replace(".md", ""), "Choose a filename.");
...
async function askForInput(value, promt, ignoreFocusOut = true) {
