...
function activate(context) {
  ...
  var commands = [
    ...
    vscode.commands.registerCommand('extension.markdown-pdf.mdEx', async function () { await markdownmdEx(); }),
...
async function markdownmdEx() {
  try {
    ...
    if (!isExistsPath(sourceFilePath)) {
      if (editor.document.isUntitled) {
        vscode.window.showWarningMessage('Please save the file!');
        return;
      }
    ...
    if (ext != ".md") {
      vscode.window.showWarningMessage('File Extension does not md!');
      return;
    }


    let sourceFileDic = path.dirname(sourceFilePath);
    ...
    if (saveFilePathUri == undefined || saveFilePathUri == null || saveFilePathUri == ""
    ...
    if (saveFilePath == sourceFilePath) {
      vscode.window.showWarningMessage('Cannot Overwrite Current File');
      return;
    }
    ...
    for (const item of mathArray) {
      ...
      let imageFileUri = path.join(sourceFileDic, imageFilePath);

      // read image file to base64
      let imageFile = fs.readFileSync(imageFileUri);
      let imageBase = imageFile.base64Slice();

      // Convert images in text to base64
      let imageTxt = item.replace(imageFilePath, "data:image/png;base64," + imageBase);
      text = text.replace(item, imageTxt);
    }

    // save file
    fs.writeFileSync(saveFilePath, text);
    vscode.window.showInformationMessage('success save ' + saveFilePath);
    return;


  } catch (error) {
    showErrorMessage('markdownmdEx()', error);
  }
