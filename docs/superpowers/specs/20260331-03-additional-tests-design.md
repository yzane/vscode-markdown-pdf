...
- `src/utils.js`: 12関数、86件のユニットテスト + Windows依存4件のpending
- `extension.js`: 統合テスト12件（HTMLスナップショット7 + バイナリ生成3 + エラーハンドリング2）
...
- ソースコード変更: 原則なし。ただし `readFile` のディレクトリ入力を空文字で扱うための最小修正は許容する
